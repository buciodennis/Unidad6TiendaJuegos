...
public class Reportes {
  ...
  public DefaultTableModel  ventasEmpleadoMes(String mes, String año){
        ...
        try{
            ...
            rs=st.executeQuery("select e.id_empleado, e.puesto, concat(e.nombre,' ', e.primerapellido,' ', e.segundoapellido) as nombre,\n" +
            "(\n" +
            "select count(v.id_venta) from ventas v  where v.id_empleado=e.id_empleado and month(v.fecha)='"+mes+"' and year(v.fecha)='"+año+"' \n" +
            ") as cantidadVentas,\n" +
            "(\n" +
            "select sum(v.monto_total) from ventas v  where v.id_empleado=e.id_empleado and month(v.fecha)='"+mes+"' and year(v.fecha)='"+año+"' \n" +
            ") as montoTotal\n" +
            "from empleados e order by montoTotal desc");
